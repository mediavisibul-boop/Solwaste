...
const SlideContent: React.FC<{ slide: Slide }> = ({ slide }) => {
  ...
  return (
    <div className={`relative w-full h-full flex flex-col justify-between ${backgroundClass}`}>
      ...
      {/* Header Content */}
      <div className={`relative z-10 container mx-auto px-4 sm:px-6 pt-28 sm:pt-32 md:pt-36 text-center`}>
        <motion.div
           initial={{ opacity: 0, y: 30 }}
           animate={{ opacity: 1, y: 0 }}
        ...
        >
            {slide.badge && (
              <span className={`inline-block py-1.5 sm:py-2 px-4 sm:px-6 mb-4 sm:mb-6 text-[10px] sm:text-xs font-bold uppercase tracking-[0.15em] sm:tracking-[0.2em] border ${isDark ? 'border-brand-gold text-brand-gold bg-brand-dark/60' : 'border-brand-olive-dark text-brand-olive-dark bg-white/60'} backdrop-blur-sm rounded-sm`}>
              {slide.badge}
            </span>
          )}
          <h1 className={`text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-heading font-bold ${textColor} mb-3 sm:mb-4 tracking-tight shadow-sm px-2`}>
            {slide.title}
          </h1>
          <h2 className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold uppercase tracking-[0.15em] sm:tracking-[0.2em] ${subColor} mb-6 sm:mb-8 opacity-90 px-2`}>
            {slide.subtitle}
          </h2>
          <p className={`text-base sm:text-lg md:text-xl lg:text-2xl ${descColor} max-w-3xl mx-auto leading-relaxed font-semibold drop-shadow-sm px-4 sm:px-6`}>
            {slide.description}
          </p>
        </motion.div>
      </div>
      ...
      {/* Footer Specs & CTA */}
      <div className="relative z-10 container mx-auto px-4 sm:px-6 pb-12 sm:pb-16 md:pb-20 lg:pb-24">
        <div className="flex flex-col md:flex-row items-end justify-between gap-6 sm:gap-8">
          {/* Specs Grid */}
          <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8 lg:gap-12 w-full md:w-auto ${hasBackgroundImage ? 'text-white' : (isDark ? 'text-white' : 'text-brand-brown')}`}>
            {slide.specs?.map((spec, idx) => (
              <div key={idx} className="flex flex-col border-l-2 sm:border-l-4 border-brand-gold/50 pl-3 sm:pl-4 md:pl-6">
                <div className="flex items-baseline">
                   <span className="text-2xl sm:text-3xl md:text-4xl font-heading font-bold">{spec.value}</span>
                   {spec.unit && <span className="text-xs sm:text-sm font-bold text-brand-gold ml-1">{spec.unit}</span>}
                </div>
                <span className={`text-[10px] sm:text-xs font-bold uppercase tracking-widest mt-1 sm:mt-2 ${hasBackgroundImage ? 'text-gray-300' : (isDark ? 'text-gray-300' : 'text-gray-600')}`}>{spec.label}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full md:w-auto mt-4 md:mt-0">
             <Link to="/contact" className="w-full sm:w-auto md:w-auto">
                <Button className={`w-full py-3 sm:py-4 px-8 sm:px-10 md:px-12 text-sm sm:text-base shadow-xl touch-manipulation ${isDark ? 'bg-white text-brand-dark hover:bg-brand-light border-transparent' : 'bg-brand-dark text-white hover:bg-brand-olive border-transparent'}`}>
                    Order Now
                </Button>
            </Link>
            <Link to="/owc" className="w-full sm:w-auto md:w-auto">
                <Button variant={isDark ? 'white' : 'outline'} className={`w-full py-3 sm:py-4 px-8 sm:px-10 md:px-12 text-sm sm:text-base backdrop-blur-sm touch-manipulation ${!isDark && 'border-brand-olive-dark text-brand-olive-dark hover:bg-brand-olive hover:text-white'}`}>
                    Tech Specs
                </Button>
            </Link>
          </div>
          ...
          <div className="flex justify-center gap-2 mt-4 sm:mt-6">
            {slide.images.map((_, idx) => (
              <button
                ...
                className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 touch-manipulation ${
                  idx === currentImageIndex
                    ? `w-6 sm:w-8 ${isDark ? 'bg-white' : 'bg-brand-dark'}`
                    : `w-1.5 sm:w-2 ${isDark ? 'bg-white/30' : 'bg-brand-dark/30'}`
                }`}
                aria-label={`View image ${idx + 1}`}
              />
            ))}
  ...
  return (
    <>
      ...
      <section className="relative h-screen overflow-hidden bg-black">
        ...
        {/* CONTROLS - Desktop Only (Mobile uses swipe) */}
        <div className="absolute top-1/2 left-2 sm:left-4 z-20 hidden md:block">
            <button 
                onClick={() => paginate(-1)} 
                className="p-4 md:p-6 bg-white/10 hover:bg-white/20 text-white rounded-full backdrop-blur-md border border-white/10 transition-all hover:scale-110 active:scale-95 group touch-manipulation"
                aria-label="Previous slide"
            >
                <ChevronLeft size={32} className="md:w-10 md:h-10 group-hover:-translate-x-1 transition-transform" />
            </button>
        </div>
        <div className="absolute top-1/2 right-2 sm:right-4 z-20 hidden md:block">
            <button 
                onClick={() => paginate(1)} 
                className="p-4 md:p-6 bg-white/10 hover:bg-white/20 text-white rounded-full backdrop-blur-md border border-white/10 transition-all hover:scale-110 active:scale-95 group touch-manipulation"
                aria-label="Next slide"
            >
                <ChevronRight size={32} className="md:w-10 md:h-10 group-hover:translate-x-1 transition-transform" />
            </button>
        </div>

        {/* BOTTOM INDICATORS */}
        <div className="absolute bottom-6 sm:bottom-8 left-1/2 transform -translate-x-1/2 z-20 flex space-x-2 sm:space-x-4">
             {slides.map((_, idx) => (
                 <button 
                    key={idx}
                    onClick={() => {
                    ...
                    className={`h-1.5 sm:h-2 rounded-full transition-all duration-500 ease-in-out touch-manipulation ${idx === currentIndex ? 'w-12 sm:w-16 bg-brand-gold' : 'w-4 sm:w-5 bg-white/30 hover:bg-white active:bg-white'}`}
      ...
      <section className="bg-brand-dark py-12 sm:py-16 md:py-20 lg:py-24 relative overflow-hidden border-t border-gray-700">
         ...
         <div className="container mx-auto px-4 sm:px-6 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-12 md:gap-16 items-center">
               <div>
                  <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-heading font-bold text-white mb-6 sm:mb-8 uppercase leading-tight">
                     Government <br/><span className="text-brand-gold">Mandate Ready</span>
                  </h2>
                  <p className="text-gray-300 text-base sm:text-lg md:text-xl mb-8 sm:mb-10 max-w-lg leading-relaxed font-normal">
                  ...
                  <div className="grid grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
                      <div>
                          <div className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold text-white mb-2">500+</div>
                          <div className="text-xs sm:text-sm text-brand-gold font-bold uppercase tracking-widest">Active Sites</div>
                      </div>
                      <div>
                          <div className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold text-white mb-2">100%</div>
                          <div className="text-xs sm:text-sm text-brand-gold font-bold uppercase tracking-widest">Legal Compliance</div>
                      </div>
                  </div>
                  <Link to="/contact" className="block sm:inline-block">
                     <Button className="w-full sm:w-auto bg-brand-gold text-white border-brand-gold hover:bg-white hover:text-brand-gold py-4 sm:py-5 px-8 sm:px-10 text-sm sm:text-base uppercase tracking-widest touch-manipulation">
               ...
               <div className="grid grid-cols-1 gap-4 sm:gap-6 mt-8 lg:mt-0">
                  {[
                    ...
                    <div key={i} className="p-6 sm:p-8 md:p-10 border border-brand-gold-dark/50 hover:border-brand-gold transition-colors bg-brand-gold-dark/20 group">
                       <h3 className="text-lg sm:text-xl md:text-2xl font-heading font-bold text-white mb-2 sm:mb-3 uppercase group-hover:text-brand-gold transition-colors">{item.title}</h3>
                       <p className="text-gray-300 text-sm sm:text-base md:text-lg leading-relaxed">{item.desc}</p>
                    </div>
                  ))}
               </div>
            </div>
      ...
      <section className="py-16 sm:py-20 md:py-24 lg:py-32 bg-brand-light border-t border-gray-200">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-12 md:gap-16 lg:gap-20 items-center mb-16 sm:mb-20 md:mb-24">
            <div>
               <div className="flex items-center space-x-3 sm:space-x-4 mb-4 sm:mb-6">
                  <div className="h-[2px] sm:h-[3px] w-8 sm:w-12 bg-brand-gold"></div>
                  <span className="text-brand-gold font-bold uppercase tracking-[0.15em] sm:tracking-[0.2em] text-xs sm:text-sm md:text-base">Our Expertise</span>
              </div>
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-heading font-bold text-brand-dark mb-6 sm:mb-8 uppercase leading-tight">German Engineering. <br/><span className="text-brand-olive-dark">Indian Innovation.</span></h2>
              <div className="space-y-4 sm:space-y-6 text-brand-dark leading-relaxed text-base sm:text-lg md:text-xl font-normal">
                <p>
              ...
              <div className="mt-8 sm:mt-10 md:mt-12">
                 <Link to="/contact" className="block sm:inline-block">
                    <Button variant="outline" className="w-full sm:w-auto px-8 sm:px-10 py-3 sm:py-4 text-xs sm:text-sm border-brand-gold-dark text-brand-gold-dark hover:border-brand-gold hover:bg-brand-gold hover:text-white uppercase tracking-widest font-bold touch-manipulation">Read More</Button>
                 </Link>
              </div>
            </div>
            <div className="relative mt-8 lg:mt-0">
                <div className="absolute top-0 right-0 -mr-4 sm:-mr-8 -mt-4 sm:-mt-8 w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 bg-white z-0 hidden sm:block"></div>
                <img 
                    src="https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?q=80&w=2070&auto=format&fit=crop" 
                    alt="Engineering Team" 
                    className="shadow-2xl relative z-10 w-full grayscale hover:grayscale-0 transition-all duration-700 border-b-4 sm:border-b-8 border-brand-gold rounded-sm"
                />
               <div className="absolute -bottom-4 sm:-bottom-8 -left-4 sm:-left-8 w-48 h-48 sm:w-56 sm:h-56 md:w-64 md:h-64 border-l-4 sm:border-l-8 border-b-4 sm:border-b-8 border-brand-dark z-20 hidden md:block"></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
            <Card 
              icon={<Target size={36} className="sm:w-12 sm:h-12" />}
              title="Our Mission"
            ...
            />
            <Card 
              icon={<Globe size={36} className="sm:w-12 sm:h-12" />}
              title="Vision 2047"
            ...
            />
            <Card 
              icon={<Award size={36} className="sm:w-12 sm:h-12" />}
